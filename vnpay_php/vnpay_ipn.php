        ...
        if ($order != NULL) {
            if($dbAmount == $$vnpAmount) //Kiểm tra số tiền thanh toán của giao dịch: giả sử số tiền kiểm tra là đúng. //$order["Amount"] == $vnp_Amount
            {
                if ($order["status"] != NULL && $order["status"] == 0) {
                    ...
                    $updateSql = "UPDATE orders SET status = ?, vnp_transaction_no = ?, vnp_response_code = ? WHERE order_id = ?";
                    $updateStmt = $conn->prepare($updateSql);
                    $updateStmt->execute([$Status, $vnp_TransactionNo, $vnp_ResponseCode, $vnp_TxnRef]);
